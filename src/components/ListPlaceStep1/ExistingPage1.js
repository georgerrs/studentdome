...
class ExistingPage1 extends Component {
  ...
  render() {
    const { nextPage, listingSteps, photosCount, stepsLoading } = this.props;
   
    if(stepsLoading){
      return <Loader type={"text"} />
    }
    const { listingSteps: { listing: { id, isReady, isPublished }}} = this.props;
        ...
        <Grid>
          <Row className={s.landingContainer}>
            <Col xs={12} sm={7} md={7} lg={7}>
              ...
              {
                listingSteps && isReady && !isPublished && <Col xs={12} sm={12} md={12} lg={12} >
